...
| `plan-guardian` | 1.3.0 | Non-blocking plan review via agent and `/plan-review` skill. **v1.3.0**: opt-out **codex review gate** — `PreToolUse:ExitPlanMode` hook blocks plan-mode exits until `/plan-guardian:codex-plan-review` loops `codex exec` against the plan and revises it until Codex emits the literal `NO_CONCERNS`; a sha256 sentinel (`./.plan-review/.codex-review-done`) confirms the exact plan was the one approved. `CODEX_PLAN_REVIEW=0` disables the gate; codex CLI missing → hook is a silent no-op. Per-round audit trail in `./.plan-review/codex-rounds/`. |
