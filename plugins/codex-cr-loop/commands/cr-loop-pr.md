...
4. **Classify the verdict** (only reached if step 3's post-review HEAD check passed and the round didn't time out):
   ...
   **4c. Hard round cap.** After classification, if `ROUND_NUM >= CR_LOOP_ROUND_CAP` (default 20): write a handoff (step 9) with `Stop reason: round-cap-reached`, surface, exit. The cap exists to bound worst-case token spend on a single `/cr-loop-pr` invocation; the user can resume by re-invoking with the cap raised (`CR_LOOP_ROUND_CAP=40 /cr-loop-pr ...`) or take the surfaced state and decide manually.
